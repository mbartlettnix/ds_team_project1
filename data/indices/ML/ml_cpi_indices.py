...
"""
to use this machine learning classifier you will need to input three features into the model. Those features are 'restaurant_price_index', 'rent_index', 'groceries_index'. The output label is 'cpi_index'
"""

df = pd.read_csv('../numbeo_ds_indices.csv')

for filename in os.listdir('../nearby_cities/'):
    df_append = pd.read_csv('../nearby_cities/{}'.format(filename))
